...
from .outbound_routing import set_agent_url
        ...
        if GENESIS_URL:
            async with ClientSession() as session:
                async with session.get(GENESIS_URL) as resp:
                    print(f"GENESIS_URL = {GENESIS_URL}")
                    genesis = await resp.text()
        elif RUN_MODE == "docker":
            async with ClientSession() as session:
                print("genesis urL : ", f"http://{DEFAULT_EXTERNAL_HOST}:9000/genesis")
...
class DemoAgent:
    def __init__(
        ...
        **params,
        ...
        print("setting agent url: ", self.admin_url)
        set_agent_url(self.admin_url)
    ...
    async def register_did(self, ledger_url: str = None, alias: str = None):
        ...
        print("LEDGER URL IS: ", ledger_url)
    ...
    async def start_process(
        ...
        print("THE ARGS FOR THE PROCESS ARE: ", agent_args)
